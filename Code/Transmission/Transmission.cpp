...
bool TransmissionHub::noise(DynamicDataBuffer& buffer)
{
    // Retourne vrai si la trame doit être oublié, faux pour garder la trame
    return m_interference->noise(buffer);
}
...
void TransmissionHub::receive_data_to_dispatch(const DynamicDataBuffer& data, Cable* from)
{
    ...
    {
        Logger log(std::cout);
        log << "Data lost - Noise drop the frame : " << std::endl;
        log << "\tFrom : " << from->getConnectedNIC().getDriver().getMACAddress() << std::endl;
    }
}