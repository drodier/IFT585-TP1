...
std::unique_ptr<Interference> Interference::CreateInterferenceImplementation(const Configuration& config)
{
    ...
    else if (noiseConfig == 2)
    {
        return std::make_unique<DropFrameInterference>(config.get(Configuration::TRANSMISSION_HUB_NOISE_FREQUENCY));
    }
    else
    {
...
bool NoInterference::noise(DynamicDataBuffer& data)
{
    return false;
}
...
bool RandomInterference::noise(DynamicDataBuffer& data)
{
...
DropFrameInterference::DropFrameInterference(unsigned int frequency)
    : m_randomGenerator()
    , m_frequency(frequency)
    , m_frequencyDistribution(0, 100)
{
...
bool DropFrameInterference::noise(DynamicDataBuffer& data)
{
    bool applyNoise = m_frequencyDistribution(m_randomGenerator) <= m_frequency;
    if (applyNoise)
    {
        Logger log(std::cout);
        log << "DropFrame Noise applied on data" << std::endl;
    }
    return applyNoise;
}